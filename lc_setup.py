...
from MISLAND.gui.WidgetLCDefineDegradation import Ui_WidgetLCDefineDegradation
from MISLAND.gui.WidgetLCSetup import Ui_WidgetLCSetup
...
class DlgCalculateLCSetAggregation(QtWidgets.QDialog, Ui_DlgCalculateLCSetAggregation):
    ...
    def btn_load_pressed(self):
        f, _ = QtWidgets.QFileDialog.getOpenFileName(self,
                                              ...
                                              QSettings().value("MISLAND/lc_def_dir", None),
                                              self.tr('Land cover definition (*.json)'))
        if f:
            if os.access(f, os.R_OK):
                QSettings().setValue("MISLAND/lc_def_dir", os.path.dirname(f))
            else:
...
class LCDefineDegradationWidget(QtWidgets.QWidget, Ui_WidgetLCDefineDegradation):
    def __init__(self, parent=None):
        ...
        self.deg_def_matrix.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.Stretch)
        self.deg_def_matrix.verticalHeader().setSectionResizeMode(QtWidgets.QHeaderView.Stretch)

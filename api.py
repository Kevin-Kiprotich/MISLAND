...
from MISLAND.worker import AbstractWorker, start_worker
...
def login(email=None, password=None):
    ...
    if (password == None):
        password = QtCore.QSettings().value("MISLAND/password", None)
    if not email or not password:
        log('API unable to login - check username/password')
        QtWidgets.QMessageBox.critical(None,
                                   QtWidgets.QApplication.translate("MISLAND", "Error"),
                                   QtWidgets.QApplication.translate("MISLAND", "Unable to login to MISLAND. Check your username and password."))
        return None

    resp = call_api('/auth', method='post', payload={"email": email, "password": password})

    if resp != None:
        QtCore.QSettings().setValue("MISLAND/email", email)
        QtCore.QSettings().setValue("MISLAND/password", password)
